...
#模型声明
model=Random_Connect(num_init_features=32,bn_size=4,num_layers=5,drop_rate=0.4,num_classes=10)
...
import csv
loss_data=[]
file=open('Random_Connect.csv','a')
...
for epoch in range(num_epoches):
    ...
    if(epoch+1)%10==0:
        # 每10个epoch保存一个模型
        torch.save(model.state_dict(), './Random_Connect_{}.pkl'.format(epoch+1))
writer.writerow(loss_data)
file.close()
