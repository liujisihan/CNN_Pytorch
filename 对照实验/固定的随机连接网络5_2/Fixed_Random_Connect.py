...
class Block(nn.Sequential):
    def __init__(self,num_layers,num_input_features,bn_size):
        ...
        #对不同的网络结构实验时，修改此处的连接列表
        # self.connect_list=[0,1,1,1,2]
        # self.connect_list=[0,1,1,1,1]
        # self.connect_list=[0,0,2,2,2]
        # self.connect_list=[0,1,0,1,1]
        self.connect_list=[0,0,1,2,2]
    ...
    def forward(self,x):
        ...
        input3=torch.cat([out2,out1],1)
        out3=self.layer3(input3)
        input4=torch.cat([out3,out1,x],1)
        out4=self.layer4(input4)
        input5=torch.cat([out4,out3,out2],1)
